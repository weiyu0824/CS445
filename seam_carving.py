...
def sobel_energy(im):
    """
    use sobel filter as energy map
    """
    filter_du = np.array([
        [1.0, 2.0, 1.0],
        [0.0, 0.0, 0.0],
    ...
    filter_dv = filter_dv * 0.1
    filter_dv = np.stack([filter_dv] * 3, axis=2)

    im = im.astype('float32')
    convolved = np.absolute(ndi.convolve(im, filter_du)) + np.absolute(ndi.convolve(im, filter_dv))
...
def get_random_seam(im, mask=None, remove_mask=None):
    """
    DP algorithm for finding the seam of minimum energy. Code adapted from 
    https://karthikkaranth.me/blog/implementing-seam-carving-with-python/
    """
    h, w = im.shape[:2]
    energyfn = forward_energy if USE_FORWARD_ENERGY else backward_energy
    # energyfn = sobel_energy
    M = energyfn(im)

    # give removal mask priority over protective mask by using larger negative value
    if remove_mask is not None:
        # print(M[np.where(remove_mask > MASK_THRESHOLD)])
        M[np.where(remove_mask > MASK_THRESHOLD)] = -ENERGY_MASK_CONST * 100

    # protect white area from being carved
    if mask is not None:
        M[np.where(mask > MASK_THRESHOLD)] = ENERGY_MASK_CONST
    ...
    for i in range(1, h):
        for j in range(0, w):
            if j == 0:
                ...
                # randomly choose the two minimum value
                # temp = np.argsort(M[i - 1, j - 1:j + 2])
                # rand = random.random()
                # idx = temp[0] if rand < 0.5 else temp[1]

                # randomly choose all three values
                idx_candidate = np.argsort( M[i - 1, j - 1:j + 2] )
                if  len(idx_candidate)==3:
                    prob = [0.4, 0.3, 0.3]
                    idx = np.random.choice(idx_candidate, 1, p=prob)[0]
                else:
                    prob = [0.5, 0.5]
                    idx = np.random.choice(idx_candidate, 1, p=prob)[0]
    ...
    # choose optical minimun seam\\
    # j = np.argmin(M[-1])
    j_candidate = np.array( np.where(M[-1] < 0)).reshape(-1) # randomly choose 
    j = np.random.choice(j_candidate, 1)[0]
...
def object_removal(im, rmask, mask=None, vis=False, horizontal_removal=False):
    ...
    output, mask = seams_insertion(output, 0, mask, vis, rot=horizontal_removal)
    if horizontal_removal:
        output = rotate_image(output, False)
...
# body
def seams_removal_b(im, num_remove, bmask, mask=None, vis=False, rot=False):
    for _ in range(num_remove):
        seam_idx, boolmask = get_random_seam(im, mask, bmask)
        if vis:
            visualize(im, boolmask, rotate=rot)
        im = remove_seam(im, boolmask)
        bmask = remove_seam_grayscale(bmask, boolmask)
        if mask is not None:
            mask = remove_seam_grayscale(mask, boolmask)
    return im, mask, bmask


def seams_insertion_b(im, num_add, bmask, mask=None, vis=False, rot=False):
    seams_record = []
    temp_im = im.copy()
    temp_bmask = bmask.copy()
    temp_mask = mask.copy() if mask is not None else None

    for _ in range(num_add):
        seam_idx, boolmask = get_random_seam(temp_im, temp_mask, temp_bmask)
        if vis:
            visualize(temp_im, boolmask, rotate=rot)

        seams_record.append(seam_idx)
        temp_im = remove_seam(temp_im, boolmask)
        temp_bmask = remove_seam_grayscale(temp_bmask, boolmask)
        if temp_mask is not None:
            temp_mask = remove_seam_grayscale(temp_mask, boolmask)
    ...
    for _ in range(num_add):
        ...
        if vis:
            visualize(im, rotate=rot)
        bmask = add_seam_grayscale(bmask, seam)
        if mask is not None:
            mask = add_seam_grayscale(mask, seam)
...
def object_body(im, dy, dx, bmask, mask=None, vis=False, horizontal_body=False):
    im = im.astype(np.float64)
    ...
    if dx < 0:
        output, mask, bmask = seams_removal_b(output, -dx, bmask, mask, vis)

    elif dx > 0:
        output, mask, bmask = seams_insertion_b(output, dx, bmask, mask, vis)

    if dy < 0:
        output = rotate_image(output, True)
        if mask is not None:
            mask = rotate_image(mask, True)
        output, mask, bmask = seams_removal_b(output, -dy, bmask, mask, vis, rot=True)
        output = rotate_image(output, False)
        ...
        if mask is not None:
            mask = rotate_image(mask, True)
        output, mask, bmask = seams_insertion_b(output, dy, bmask, mask, vis, rot=True)
        output = rotate_image(output, False)


    # if horizontal_body:
    #     output = rotate_image(output, True)
    #     bmask = rotate_image(bmask, True)
    #     if mask is not None:
    #         mask = rotate_image(mask, True)

    # while len(np.where(bmask > MASK_THRESHOLD)[0]) > 0:
    #     seam_idx, boolmask = get_minimum_seam(output, mask, bmask)
    #     if vis:
    #         visualize(output, boolmask, rotate=horizontal_body)            
    #     output = remove_seam(output, boolmask)
    #     bmask = remove_seam_grayscale(bmask, boolmask)
    #     if mask is not None:
    #         mask = remove_seam_grayscale(mask, boolmask)

    # num_add = (h if horizontal_body else w) - output.shape[1]
    # output, mask = seams_insertion(output, num_add, mask, vis, rot=horizontal_body)
    # if horizontal_body:
    #     output = rotate_image(output, False)
...
if __name__ == '__main__':
    ...
    # add
    ap.add_argument("-bmask", help="Path to body mask")
    ...
    # add
    ap.add_argument("-hbody", help="Using horizontal seams for object body", action='store_true')
    ap.add_argument("-backward_energy", help="Use backward energy map (default is forward)", action='store_true')
    args = vars(ap.parse_args())

    IM_PATH, MASK_PATH, OUTPUT_NAME, R_MASK_PATH, B_MASK_PATH = args["im"], args["mask"], args["out"], args["rmask"], args["bmask"]

    im = cv2.imread(IM_PATH)
    assert im is not None
    mask = cv2.imread(MASK_PATH, 0) if MASK_PATH else None
    rmask = cv2.imread(R_MASK_PATH, 0) if R_MASK_PATH else None
    bmask = cv2.imread(B_MASK_PATH, 0) if B_MASK_PATH else None
    ...
    if SHOULD_DOWNSIZE and w > DOWNSIZE_WIDTH:
        ...
        if bmask is not None:
            bmask = resize(bmask, width=DOWNSIZE_WIDTH)
...
# body need scale(dx, dy), bmask(body mask), (opt)mask(protective mask)
# python seam_carving.py -body -im test/man/man.jpg -out output/man/man_body.jpg
#        -dx 5 -vis -bmask test/man/man_mask_1.jpg -mask test/man/man_mask_2.jpg
    elif args["body"]:
        dy, dx = args["dy"], args["dx"]
        assert dy is not None and dx is not None
        assert bmask is not None
        output = object_body(im, dy, dx, bmask, mask, args["vis"], args["hbody"])
        cv2.imwrite(OUTPUT_NAME, output)
